...
class Settings(BaseSettings):
    ...
    # Optional online AI router. Leave the key empty to remain fully offline.
    AI_ROUTER_BASE_URL: str = ""
    AI_ROUTER_API_KEY: str = ""
    AI_ROUTER_MODEL: str = ""
    AI_ROUTER_TIMEOUT_SECONDS: float = 20.0
    AI_ROUTER_MAX_OUTPUT_TOKENS: int = 1200
    AI_ROUTER_ALLOW_ONLINE: bool = False
...
if settings.ENVIRONMENT == "production":
    if len(settings.SECRET_KEY) < 64: raise ValueError("SECRET_KEY must be at least 64 chars in production.\nGenerate: openssl rand -hex 64")
    if settings.SECRET_KEY == "change-me-in-production": raise ValueError("Change default SECRET_KEY before deploying to production!")
    try: bank_key = base64.urlsafe_b64decode(settings.BANK_ENCRYPTION_KEY.encode())
    except Exception as exc: raise ValueError("BANK_ENCRYPTION_KEY must be URL-safe base64") from exc
    if len(bank_key) != 32: raise ValueError("BANK_ENCRYPTION_KEY must decode to exactly 32 bytes")
    if settings.DATABASE_URL.startswith("postgresql+asyncpg://mir_user:change_me@"): raise ValueError("DATABASE_URL contains the development placeholder in production")
    if "localhost" in settings.DOMAIN or "localhost" in settings.FRONTEND_URL: raise ValueError("DOMAIN/FRONTEND_URL cannot use localhost in production")
    if settings.DEBUG: raise ValueError("DEBUG must be false in production")
