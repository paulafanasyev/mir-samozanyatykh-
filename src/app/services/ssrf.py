...
MIR Samozanyatykh v8.4.3 - Security Hardened
...
class SSRFProtector:
    ...
    def validate_url(cls, url: str, allow_internal: bool = False) -> tuple[bool, Optional[str]]:
        ...
        # Reject userinfo and alternate numeric IP encodings before DNS resolution.
        ...
        if hostname_lower.isdigit():
            try:
                ...
                if not ip.is_global:
                    return False, "Non-public numeric IP is blocked"
            except ValueError:
                return False, "Invalid numeric hostname"
        if hostname_lower.startswith(("0x", "0o")):
                ...
                try:
                    ...
                    for _, _, _, _, sockaddr in addr_info:
                        ...
                        try:
                            ...
                            if not ip.is_global:
                                return False, f"IP {ip_str} is not globally routable"
