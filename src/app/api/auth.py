...
async def login(
    ...
    if mfa and mfa.is_enabled:
        ...
        return {
            "requires_2fa": True,
            "temp_token": create_2fa_pending_token({"sub": str(user.id), "email": user.email})[0],
        }
    ...
    if not valid:
        ...
        for index, code_hash in enumerate(backup_codes):
            if verify_password(code, code_hash):
                ...
                await db.commit()
                valid = True
                await log_audit(action="2fa_backup_code_used", user_id=user_id)
                break
    if not valid:
...
async def refresh_token(
    ...
    try:
        ...
        if not session or session.token_type != "refresh" or session.expires_at <= datetime.now(timezone.utc):
            raise HTTPException(status_code=401, detail="Refresh token отозван или истёк")
        if session.revoked:
            await db.execute(update(UserSession).where(UserSession.user_id == user_id, UserSession.token_type == "refresh").values(revoked=True))
            await db.commit()
            raise HTTPException(status_code=401, detail="Обнаружено повторное использование refresh token")
        ...
        if not _is_mobile_client(request):
            response.set_cookie("refresh_token", new_refresh, httponly=True, secure=settings.ENVIRONMENT == "production", samesite="strict", path="/api/auth/refresh", max_age=settings.REFRESH_TOKEN_EXPIRE_DAYS * 86400)
            response.set_cookie("csrf_token", csrf_token, httponly=False, secure=settings.ENVIRONMENT == "production", samesite="strict", path="/", max_age=settings.REFRESH_TOKEN_EXPIRE_DAYS * 86400)
        return TokenResponse(
            access_token=new_access,
            refresh_token=new_refresh if _is_mobile_client(request) else None,
            token_type="bearer",
            expires_in=settings.ACCESS_TOKEN_EXPIRE_MINUTES * 60,
            tier=user.subscription_tier,
        )
...
async def confirm_password_reset(
    ...
    now = datetime.now(timezone.utc)
    result = await db.execute(select(User).where(
        User.password_reset_token_hash == hash_token(request.token),
