...
from app.core.file_security import private_storage_path, ensure_within_private_storage
...
class YookassaError(Exception): pass
...
async def create_invoice(
    ...
    # Генерация номера счёта без race condition: count+1 не гарантирует уникальность
    # при двух параллельных запросах.
    today = datetime.now(timezone.utc)
    invoice_number = f"СЧ-{current_user.id}-{today.strftime('%Y%m%d')}-{uuid4().hex[:8].upper()}"
...
async def send_invoice(
    ...
    if invoice.client_id:
        client_result = await db.execute(
            select(Client).where(Client.id == invoice.client_id, Client.user_id == current_user.id)
        )
    ...
    # Обновление статуса
    invoice.status = "sent"
    invoice.pdf_path = private_storage_path("invoices", f"{invoice.invoice_number}.pdf", current_user.id)
    await db.commit()
...
async def get_invoice_pdf(
    ...
    if invoice.pdf_path:
        import os
        safe_pdf = ensure_within_private_storage(invoice.pdf_path)
        if safe_pdf.is_file():
...
async def create_payment(
    ...
    if not idempotency_key or len(idempotency_key) > 255:
        raise HTTPException(status_code=400, detail="Требуется корректный Idempotency-Key")
    ...
    if existing_payment:
        if existing_payment.invoice_id != invoice_id:
            raise HTTPException(status_code=409, detail="Idempotency-Key уже использован для другого счёта")
    ...
    if invoice.status in ("paid", "cancelled"):
        raise HTTPException(status_code=400, detail="Счёт уже закрыт для оплаты")
    if amount <= 0:
    ...
    if total_paid > invoice.total_amount:
        await db.rollback()
        raise HTTPException(status_code=400, detail="Сумма платежей превышает сумму счёта")
    if total_paid == invoice.total_amount:
        invoice.status = "paid"
...
async def yookassa_webhook(
    ...
    invoice = result.scalar_one_or_none()
    if not invoice:
        logger.error("Yookassa webhook: invoice not found")
        raise HTTPException(status_code=404, detail="Invoice not found")

    if invoice.yookassa_payment_id and invoice.yookassa_payment_id != payment_id:
        raise HTTPException(status_code=409, detail="Payment does not match invoice")
    
    if status == "succeeded":
        amount = Decimal(str(payment_obj.get("amount", {}).get("value", 0)))
        if amount <= 0 or amount != Decimal(str(invoice.total_amount)):
            raise HTTPException(status_code=400, detail="Сумма платежа не соответствует счёту")

        existing = await db.scalar(select(Payment).where(Payment.yookassa_id == payment_id, Payment.invoice_id == invoice.id))
        if existing:
        ...
        await db.commit()
        logger.info("Yookassa payment cancelled: %s", payment_id)
