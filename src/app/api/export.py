...
async def export_excel(module: str, db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user)):
    """Универсальный Excel экспорт пользовательских данных."""
    from openpyxl import Workbook
    from fastapi.responses import Response
    models = {"products": Product, "invoices": Invoice, "clients": Client, "deals": Deal, "tasks": Task}
    model = models.get(module)
    if not model:
        raise HTTPException(status_code=404, detail="Модуль экспорта не найден")
    result = await db.execute(select(model).where(model.user_id == current_user.id))
    rows = result.scalars().all()
    wb = Workbook(); ws = wb.active; ws.title = module[:31]
    if rows:
        columns = [c for c in model.__table__.columns.keys() if c != "user_id"]
        ws.append(columns)
        for row in rows:
            ws.append([getattr(row, c, None) for c in columns])
    else:
        ws.append(["message"]); ws.append(["Нет данных"])
    buf = io.BytesIO(); wb.save(buf); buf.seek(0)
    return Response(buf.getvalue(), media_type="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", headers={"Content-Disposition": f"attachment; filename={module}.xlsx"})

@router.get("/{module}/pdf")
async def export_pdf(module: str, db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user)):
    """Универсальный PDF экспорт с базовой таблицей."""
    ...
    result = await db.execute(select(model).where(model.user_id == current_user.id))
    rows = result.scalars().all()
    columns = [c for c in model.__table__.columns.keys() if c != "user_id"][:8]
    data = [columns]
    for row in rows[:500]:
        data.append([str(getattr(row, c, "") or "")[:120] for c in columns])
    if len(data) == 1: data.append(["Нет данных"] + [""] * (len(columns)-1))
    buf=io.BytesIO(); doc=SimpleDocTemplate(buf,pagesize=A4,rightMargin=24,leftMargin=24,topMargin=24,bottomMargin=24)
    doc.build([Paragraph(f"Экспорт: {module}", getSampleStyleSheet()["Heading2"]), Table(data, repeatRows=1, style=TableStyle([("GRID",(0,0),(-1,-1),0.25,colors.grey),("BACKGROUND",(0,0),(-1,0),colors.lightgrey)]))])
    return StreamingResponse(io.BytesIO(buf.getvalue()), media_type="application/pdf", headers={"Content-Disposition": f"attachment; filename={module}.pdf"})
