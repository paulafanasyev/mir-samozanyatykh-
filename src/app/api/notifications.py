...
async def send_bulk_notification(
    ...
    # Не загружаем и не отправляем массовую рассылку без верхней границы:
    # это защищает endpoint от случайного OOM и слишком больших синхронных операций.
    total_query = select(func.count(User.id)).where(User.is_active == True)
    if bulk.target_tier:
        total_query = total_query.where(User.subscription_tier == bulk.target_tier)
    total_users = await db.scalar(total_query) or 0
    max_recipients = 5000
    if total_users > max_recipients:
        raise HTTPException(
            status_code=422,
            detail=f"Слишком много получателей ({total_users}). Используйте сегментацию.",
        )
    ...
    # Сначала атомарно сохраняем in-app уведомления. Email не должен
    # отправиться, а затем исчезнуть из БД из-за rollback.
    for user in users:
    ...
    # Email — внешний side effect; ошибки email не откатывают уже сохранённые уведомления.
    if bulk.send_email:
        for user in users:
            if not user.email:
                continue
