...
async def create_deal(
    ...
    """Создание сделки"""
    # Проверка клиента и этапа принадлежат одному пользователю
...
async def create_call(
    ...
    if call_data.get("client_id") is not None:
        client_result = await db.execute(
            select(Client).where(
                Client.id == call_data["client_id"],
                Client.user_id == current_user.id,
            )
        )
        if not client_result.scalar_one_or_none():
...
async def create_task(
    ...
    if task_data.get("client_id") is not None:
        client_result = await db.execute(
            select(Client).where(
        ...
        if not client_result.scalar_one_or_none():
            raise HTTPException(status_code=404, detail="Клиент не найден")
    if task_data.get("deal_id") is not None:
        ...
        if not deal_obj:
            raise HTTPException(status_code=404, detail="Сделка не найдена")
        if task_data.get("client_id") is not None and deal_obj.client_id != task_data["client_id"]:
        ...
        try:
            if auto.action_type == "send_notification":
                ...
                if client_id is not None:
                    ...
                    if client is None:
                        raise ValueError("Automation client does not belong to user")
                if deal_id is not None:
                    deal = await db.scalar(select(Deal).where(Deal.id == deal_id, Deal.user_id == user_id))
                    if deal is None:
                        raise ValueError("Automation deal does not belong to user")
                    if client_id is not None and deal.client_id != client_id:
                        raise ValueError("Automation task client does not match deal client")
                task = Task(
                    user_id=user_id,
                ...
                if deal_id and stage_id:
                    ...
                    if deal:
                        ...
                        if stage is None:
                            raise ValueError("Automation stage does not belong to user")
                        deal.stage_id = stage_id
