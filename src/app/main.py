...
_known_render_host="mir-samozanyatykh-api-frankfurt.onrender.com"; _allowed_hosts=[]
for _host in (settings.DOMAIN, os.getenv("RENDER_EXTERNAL_HOSTNAME", ""), _known_render_host):
    _host=_host.strip().lower()
    if _host.startswith("https://"): _host=_host[8:]
    elif _host.startswith("http://"): _host=_host[7:]
    _host=_host.rstrip("/")
    if _host and _host not in _allowed_hosts: _allowed_hosts.append(_host)
if settings.ENVIRONMENT != "production": _allowed_hosts.extend(["localhost","127.0.0.1"])
...
async def security_headers(request: Request, call_next):
    ...
    if request.url.path.startswith("/static/svetlana/"): response.headers["Cache-Control"]="public, max-age=31536000, immutable"
...
async def request_logging(request: Request, call_next):
    ...
    try:
        response=await call_next(request); duration=(time.time()-start_time)*1000; record_request(response.status_code,duration/1000.0)
        logger.info(f"{request.method} {request.url.path} {response.status_code} {duration:.2f}ms {request.client.host}",extra={"method":request.method,"path":request.url.path,"status":response.status_code,"duration_ms":round(duration,2),"ip_address":request.client.host,"user_agent":request.headers.get("user-agent","")[:100]})
        return response
...
async def root(request: Request): return {"app":settings.APP_NAME,"version":settings.APP_VERSION,"status":"running","environment":settings.ENVIRONMENT}
