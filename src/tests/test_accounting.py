...
async def accounting_user(db_session: AsyncSession):
    await db_session.execute(delete(User).where(User.email == "accounting_test@example.com"))
...
class TestDashboardCalculations:
    ...
    async def test_income_calculation(self, db_session: AsyncSession, accounting_user: User, sample_transactions):
        ...
        # Clean up transactions from other tests
        await db_session.execute(delete(Transaction).where(Transaction.user_id == accounting_user.id))
        await db_session.commit()
        # Re-add sample transactions (need to make them transient first since they were deleted)
