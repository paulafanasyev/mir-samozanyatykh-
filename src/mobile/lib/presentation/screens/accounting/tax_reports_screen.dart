...
class _TaxReportsScreenState extends ConsumerState<TaxReportsScreen> {
  ...
  Future<void> _loadTaxData() async {
    try {
      final response = await ApiClient().getTaxReports();
      final reports = response.data is List ? List<dynamic>.from(response.data) : <dynamic>[];
      final totalIncome = reports.fold<num>(0, (sum, r) => sum + (num.tryParse('${r['total_income'] ?? 0}') ?? 0));
      final totalTax = reports.fold<num>(0, (sum, r) => sum + (num.tryParse('${r['tax_amount'] ?? 0}') ?? 0));
      final totalDeduction = reports.fold<num>(0, (sum, r) => sum + (num.tryParse('${r['deduction_total'] ?? 0}') ?? 0));
      setState(() {
        taxData = {'reports': reports, 'total_income': totalIncome, 'total_tax': totalTax, 'total_deduction': totalDeduction};
        isLoading = false;
      });
    } catch (e) {
