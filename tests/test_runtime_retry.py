...
class RuntimeRetryTests(unittest.TestCase):
    def test_runtime_failure_is_retryable_and_does_not_pass(self):
        ...
        def security(task, previous):
            ...
            return AgentResult(task.id, task.role, TaskStatus.PASSED, "ok", evidence=["static-security:checked:app/main.py"], metadata={"verification_level": "static"})

        def qa(task, previous):
            from app.agents.models import AgentResult
            return AgentResult(task.id, task.role, TaskStatus.PASSED, "ok", evidence=["qa:unittest:exit:0"], metadata={"verification_level": "static"})
