...
class HttpProviderTests(unittest.TestCase):
    def test_missing_key_fails_closed(self):
        with self.assertRaises(ProviderUnavailable):
            openai_compatible_call("https://example.invalid/v1", "", "model", "hello", 1)
    ...
    def test_response_is_normalized(self, post):
        ...
        result = openai_compatible_call("https://example.test/v1", "secret", "model", "hello", 1)
        self.assertEqual(result.model, "model")
        self.assertEqual(result.text, "hello")
        self.assertEqual(result.provider, "https://example.test/v1")
    ...
    def test_429_is_rate_limited(self, urlopen):
        ...
        with self.assertRaises(ProviderRateLimited):
            openai_compatible_call("https://example.test/v1", "secret", "model", "hello", 1)
