      ...
      <div className="flex flex-wrap items-center gap-3">
        <div className="flex bg-white border border-slate-200 rounded-lg overflow-hidden">
          {[{ k: 'all', l: 'Vse' }, { k: 'income', l: 'Dokhody' }, { k: 'expense', l: 'Raskhody' }].map(t => (
            <button key={t.k} onClick={() => setFilterType(t.k as any)} className={`px-4 py-2 text-sm font-medium ${filterType === t.k ? 'bg-blue-50 text-blue-700' : 'text-slate-600 hover:bg-slate-50'}`}>
              {t.l}
            </button>
          ))}
        </div>
